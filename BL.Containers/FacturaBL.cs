...
namespace BL.Containers
{
    public class FacturaBL
    {
        ...
        private void CalcularExistencia(Factura factura)
        {
            ...
            {
                var container = _contexto.container.Find(detalle.ContainerId);
                if (container != null)
                {
                    if (factura.Activo == true)
                    {
                        container.Existencia = container.Existencia - detalle.Cantidad;
                    }
                    else
                    {
                        container.Existencia = container.Existencia + detalle.Cantidad;
        ...
        private Resultado Validar(Factura factura)
        {
            ...
            {
                resultado.Mensaje = "Agregue containers a la factura";
                resultado.Exitoso = false;
            }

            foreach (var detalle in factura.FacturaDetalle)
            {
                if (detalle.ContainerId == 0)
                {
                    resultado.Mensaje = "Seleccione containers validos";
                    resultado.Exitoso = false;
                }
            }
        ...
        public void CalcularFactura(Factura factura)
        {
            if (factura != null)
            {
                ...
                foreach (var detalle in factura.FacturaDetalle)
                {
                    ...
                    if (container != null)
                    {
                        detalle.Precio = container.Precio;
                        detalle.Total = detalle.Cantidad * container.Precio;
